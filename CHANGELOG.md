...
## [Unreleased]
### Fixed
- **Withdrawal flags now survive mixed-event dedup.** A data refresh was rejected because the fresh scrape flagged both copies of a withdrawal bout in a twice-listed mixed event, so the raw flagged-row count no longer matched the deduped output and validation failed. The dedup pass (`dedupeBouts`, extracted from `processBouts`) is now flag-aware: a dropped duplicate's withdrawal flag is merged onto the kept row, so whether a withdrawal survives no longer depends on which copy the scraper emitted first. The dataset smoke test and `diagnose-withdrawals.mjs` compute the expected count from the deduped rows via the same helper; two new unit tests cover both flag orderings

### Added
- **API-layer test coverage** (`frontend/test/api.test.js`, 55 tests). Pins the canonical bout-fingerprint wire format and asserts parity between the server (`api/_lib.js`) and client (`src/data/edits.js`) implementations; golden vectors for licence normalisation and hashing mirroring the Python ingest (`normalise_licence`/`hash_licence`), so a drift that would silently break every login now fails a test; the admin token gate (verified to fail closed in every misconfiguration case); edit validation and alias-ownership resolution; merge chain collapse, self-merge rejection, and unmerge. Pure logic was extracted from the `edit.js`/`admin.js` handlers into exported helpers (behaviour-preserving) to make it testable without mocking Redis

### Changed
- **Keyboard and screen-reader accessibility across the public site.** Every navigation control that was a clickable `div`/`span` (fencer, club, and competition links, nav tabs, sort headers, back controls, search and picker results) is now a real `<button>` or a focusable pressable row with Enter/Space activation; a shared `:focus-visible` outline gives one visible keyboard-focus treatment everywhere; the active nav tab carries `aria-current`. The sign-in modal gained dialog semantics (`role="dialog"`, `aria-modal`, labelled title), a focus trap, Escape-to-close, focus return to the opener, a labelled input, and announced errors. The bout dispute flow's `prompt()` was replaced with an inline reason form (focusable textarea, Escape cancels, live status updates). DE tableau fencer names are focusable and tracing a line works from the keyboard exactly like hover
- **Site FAQ copy finalised.** The Method tab FAQ shipped marked as a working draft; the draft banner and placeholder framing are gone and the copy is kept in step with the README FAQ
- **One-button data refresh** in `/#admin` (new Refresh tab). "Refresh from FeNZ" re-runs the ingest against the FeNZ API, regenerates `ingest/bouts.csv`, and commits it, which redeploys the live site, so keeping the data current no longer means running the Python script and committing by hand. The work runs in a new `refresh-data` GitHub Actions workflow triggered by `repository_dispatch` from a new admin-gated function (`frontend/api/refresh.js`); the dataset stays a committed-CSV artefact, so the canonical "commit the regenerated file" path is unchanged, just automated. Two guards run before anything ships: a shrink-guard (new `ingest/diff_bouts.py`) aborts the run if the rebuilt file lost more than 10% of its rows (the failure mode when the cmpId scan range is set too high and silently drops early competitions), and the existing test suite runs against the regenerated dataset. Because it auto-deploys, the safety net is after the fact rather than a gate: every run is a row in a log (the recent commit history of `bouts.csv`, so the audit trail is just git); an admin can Accept a refresh or Revert it (restore the previous `bouts.csv` and redeploy). Setup is one fine-grained GitHub token in Vercel env, documented in SETUP.md Phase 5

- **Failure diagnostics for `refresh-data`.** When a refresh's validation step rejects the regenerated dataset, the run now explains why instead of just failing the test. A new `frontend/scripts/diagnose-withdrawals.mjs` reproduces `processBouts` and names any withdrawal-flagged input row that didn't surface as a withdrawal bout, with the likely cause (blank/duplicate name, self-bout, non-numeric score, dedup collision, or missing rating snapshot); the workflow runs it on failure and uploads the rejected `bouts.csv` as an artifact, so the offending row is visible in the log and the data is recoverable for offline analysis (it otherwise only exists on the runner). Failure-path only, so the validation gate is unchanged
...
### Fixed
- Activity-implausible dates of birth no longer win the registry lookup. When a fencer is split into two records by a data-entry typo on one DOB, `buildEnrichmentIndex` could surface the wrong year: one senior fencer's typo'd record carried a birth year that would make her six at the time of a national foil ranking she holds, and that record winning the lookup had placed her in the Junior and Cadet leaderboards. A DOB that would make the fencer younger than 8 at the time of any ranking they hold is now demoted. The guard is deliberately narrow (exactly one record in the registry trips it), so genuine namesakes, like two fencers who share a name but were born twenty years apart, stay plausible and unchanged
...
- Secondary Schools events were classified as Senior. They match no U-band keyword (`U15`, `cadet`, `junior`, …) so `parseAgeCategory` fell them through to `senior`, feeding their bouts into the Senior/Junior/Cadet streams and able to make a school-only fencer a "native Senior". They're now classified as Cadet, the conservative choice for an NZ school-age field (years 9–13), so the bouts never count upward. Investigation note: the Senior rating *stream* itself was never contaminated by younger results (downward inclusion only flows Senior→Junior→Cadet), so the broader "U17s rated high" perception in FEEDBACK Round 3 is mostly the default "All ages" view blending categories, which is a separate UX item
- Sibling DOB collisions in the licence registry. A shared or mis-entered licence number in the source Fencing Time XML can fold a stray name alias of one family member into another's record, so one fencer's name key ended up living on his sibling's registry record. `buildEnrichmentIndex` resolved that key to whichever record had the most signal, and the club-count tiebreaker handed it to the sibling, surfacing her birth year instead of his (which also wrongly dropped him from the Junior leaderboard; the 0.1.10 note recorded that drop as correct). The lookup now gives a record an ownership bonus for a `name_key` equal to its own `display_name`, so a fencer always wins their own name key over a sibling listing it as an alias. A registry scan found and corrected six affected fencers
### Added
...
## [0.1.10] - 2026-05-19
### Added
- DOB-based eligibility for the Cadet and Junior leaderboards: when the fencer is in the XML registry, age category derives from `dob_year` directly. Junior cutoff is `currentYear - 20` (born 2006+ in the 2026 season), Cadet is `currentYear - 17` (born 2009+). Auto-rolls each year as `currentYear` shifts. Fencers without a DOB still pass through the old event-tag inference. Net effect on Mens Épée Junior: list grows 55 → 68; one fencer drops out on a registry birth year (later found to be a sibling's, corrected in 0.1.12), and ~15 juniors correctly appear despite skipping the 2026 junior-tagged events
- Weapon-aware Clubs page that respects the global weapon pill. The Mens Épée view now ranks clubs by their Mens Épée DE median; clubs whose entire roster is in other weapons (NZ Academy of Fencing: 29 members, all foil/sabre) no longer top the list with 0 épéeists
...
- Plausibility filter in `fencerinfo_ingest.py` rejects DOBs that would make a fencer younger than 7 in the current year. Caught 14 XML data-entry errors (an adult fencer recorded as "born 2026", and similar) that would otherwise false-positive into Cadet
- Visual sort indicators (`↓` arrow + active-row bolding) on the Bouts and W·L Leaderboard headers. The sort maths always worked but the active-state cue was missing
### Changed
- Strength-tier letter on the Clubs page now requires at least 3 members active in the selected weapon. Under-threshold clubs show a dash instead of A/B/C, so a 1-fencer outpost with a single star can't outrank a deep club via median games. Partially addresses FEEDBACK Round 2 item #1 (club strength tier favoured tiny clubs)
- Activity gate on DOB-based junior/cadet filtering was dropped: being age-eligible is enough. Previously a junior who hadn't entered the 2026 season was excluded; now age-eligible juniors whose last épée event was in 2024 or 2025 correctly appear in the 2026 junior leaderboard. The no-DOB fallback path still enforces recent activity as a sanity gate
...
- Override-doc edit key now matches the bout-derived fencer key. The client sends its own `fencer_key` with edits and the server verifies it's an alias of the licenced fencer before honouring. Fixes a bug where overrides for a fencer with multiple name aliases wrote under the alphabetically-first alias and were silently never read on the public site
...
- Dispute bout flow: a "Dispute" button per bout row on your own profile (signed-in only) opens a reason prompt, submits a dispute that immediately flags the bout fingerprint publicly (small `flagged` badge on the row) and enqueues an admin review item
...
- Visual sort indicators (`↓` arrow + cell bolding) on the Bouts and W·L Leaderboard columns, matching the existing Pool and DE feedback. The underlying sort maths already worked but had no clear active-state cue
...
- `App.jsx` layers Upstash overrides over the bout-derived `fencers` map after `processBouts` runs (name + club overrides) and threads a `flagged_bouts` Set into `FencerProfile`. Rating maths is unaffected by edits in this release
...
- Competition list now sorts chronologically when CSVs use `D/M/YYYY` or `M/D/YYYY` instead of ISO. `processBouts` detects the date format from the dataset and normalises all dates to `YYYY-MM-DD` before sorting or formatting
...
- Displayed rating is now `rating − k × RD` (TrueSkill-style conservative rating) across the Leaderboard, fencer profile headline numbers, weapon pills, rating-progression chart, and the head-to-head fencer picker. A fencer with `1800 ± 300` shows as 1500 and climbs towards 1800 as their RD shrinks
...
### Notes
- Per-bout before/after deltas (FencerProfile bout rows, CompetitionDetail) and competition median/top remain raw, since those snapshots don't carry an RD alongside the rating
...
## [0.1.2] - 2026-05-04
### Fixed
- Fencers with no recorded gender no longer appear in both Men's and Women's categories. The gender filter now requires an explicit match in Leaderboard, Competitions, and FencerPicker
...
Initial release.
- Built the prototype of the app using Claude Code
- Made minor design decisions such as the elo system and styling
- Ran into issues pulling CSV results from FencingTimeLive, so looking for a workaround
